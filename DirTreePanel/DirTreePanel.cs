	...
	public class DirTreePanel : UserControl
	{
		...
		*/
		...
		public void InitializDirTreeView()
		{
			...
			this.dirTreeView1.emacstoolStripMenuItem2.Click += new System.EventHandler(this.emacstoolStripMenuItem2_Click);
			this.dirTreeView1.geditoolStripMenuItem2.Click += new System.EventHandler(this.geditoolStripMenuItem2_Click);

			this.dirTreeView1.eogtoolStripMenuItem2.Click += new System.EventHandler(this.eogtoolStripMenuItem2_Click);
			this.dirTreeView1.totemtoolStripMenuItem2.Click += new System.EventHandler(this.totemtoolStripMenuItem2_Click);
		...
		private void dirTreeView1_DoubleClick(object sender, System.EventArgs e)
		{
			...
			if (AntPanel.IsRunningUnix) path = path.Replace("\\", "/");
			...
			try
			{
				...
				if (AntPanel.IsRunningUnix)
				{
					...
					if (Lib.IsTextFile(path)) Process.Start("/usr/bin/emacs24", path);
					else if (Lib.IsImageFile(path)) Process.Start("eog", path);
					else if (Lib.IsSoundFile(path) || Lib.IsVideoFile(path)) Process.Start("totem", path);
		...
		{
			if (AntPanel.IsRunningWindows) ProcessHandler.Run_Sakura(this.dirTreeView1.filepath);
			else if (AntPanel.IsRunningUnix) Process.Start("/home/kazuhiko/bin/sakura.sh", this.dirTreeView1.filepath);
		}

		// dirTreeView1 
		private void dirTreeView1_pSPadToolStripMenuItem1_Click(object sender, EventArgs e)
		{
			if (AntPanel.IsRunningWindows) ProcessHandler.Run_PSPad(this.dirTreeView1.filepath);
			else if (AntPanel.IsRunningUnix) Process.Start("/home/kazuhiko/bin/pspad.sh", this.dirTreeView1.filepath);
		}
		...
		private void dirTreeView1_コマンドプロンプトToolStripMenuItem1_Click(object sender, EventArgs e)
		{
			...
			if (Directory.Exists(this.dirTreeView1.filepath)) dir = this.dirTreeView1.filepath;
			else if (File.Exists(this.dirTreeView1.filepath)) dir = Path.GetDirectoryName(this.dirTreeView1.filepath);
			if(!String.IsNullOrEmpty(dir))
			{
				if (AntPanel.IsRunningWindows) ProcessHandler.Run_Cmd(dir);
				else if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/gnome-terminal", dir);
			}
		}
		// dirTreeView1 
		...
		private void dirTreeView1_エクスプローラToolStripMenuItem1_Click(object sender, EventArgs e)
		{
			...
			if (AntPanel.IsRunningWindows) Process.Start(@"C:\windows\explorer.exe", "/select," + this.dirTreeView1.filepath);
			else if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/nautilus", dir);
		}
		// dirTreeView1 
		...
		private void dirTreeView1_開くtoolStripMenuItem1_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(Lib.OutputError(exc.Message.ToString()), "開くtoolStripMenuItem1_Click");
			}
		}

		private void dirTreeView1_ブラウザで開くtoolStripMenuItem1_Click(object sender, EventArgs e)
		{
				String url = this.dirTreeView1.filepath.ToLower().Replace(@"f:\", "http://localhost/f/");
		...
		private void emacstoolStripMenuItem2_Click(object sender, EventArgs e)
		{
			if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/emacs", this.dirTreeView1.filepath);
		}
		private void geditoolStripMenuItem2_Click(object sender, EventArgs e)
		{
			if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/gedit", this.dirTreeView1.filepath);
		}

		private void eogtoolStripMenuItem2_Click(object sender, EventArgs e)
		{
			if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/eog", this.dirTreeView1.filepath);
		}
		private void totemtoolStripMenuItem2_Click(object sender, EventArgs e)
		{
			if (AntPanel.IsRunningUnix) Process.Start("/usr/bin/totem", this.dirTreeView1.filepath);
		}

		private void ヘルプLToolStripButton_Click(object sender, EventArgs e)
		{
			String path = "http://192.168.0.13/f/VCSharp/Flashdevelop5.1.1-LL/External/Plugins/DirTreePanel/doxygen/html/index.html";
			//PluginBase.MainForm.CallCommand("Browse", path);
			//Process.Start(settings.win_ChromePath, path);
			Globals.MainForm.OpenCustomDocument("Browse", path);
		}
		...
		private void ShowNodeInfo(object sender, EventArgs e)
		{
			this.Controls.Remove(this.dirTreeView1);
			this.dirTreeView1.Dispose();
			if(Directory.Exists(this.dirTreeView1.filepath))
