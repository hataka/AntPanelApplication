  ...
  public class ActionManager
  {
    ...
    public static string ProcessVariable(string strVar)
    {
      ...
      try
      {
        ...
        arg = arg.Replace("$(CurProjectName)", AntPanel.projectName);
        ...
        arg = arg.Replace("$(CurFile)", GetCurrentDocumentPath());
        arg = arg.Replace("$(CurFilename)", Path.GetFileName(GetCurrentDocumentPath()));
        arg = arg.Replace("$(CurFilenameNoExt)", Path.GetFileNameWithoutExtension(GetCurrentDocumentPath()));
    ...
    public static void NodeAction(String agstring)
    {
      ...
      if (String.IsNullOrEmpty(agstring)) return;
      try
      {
        String[] tmp0 = agstring.Split('@');
    ...
    public static void NodeAction(NodeInfo ni)
    {
      ...
      {
        path = ProcessVariable(Path.Combine(pathbase,ProcessVariable(ni.Path)));
      }
      else   path = ProcessVariable(ni.Path);
      ...
      // FIXME
      //ApplyPropertySetting();

      menuTree.antPanel.StatusBar.Tag = ni.Action.ToLower() + "!" + command + "|" + args + "|" + path + "|" + option;
      ...
      try
      {
        ...
        else
        {
          ...
          if (option.ToLower().IndexOf("doc") >= 0)
          {
            ...
            ((RichTextBox)AntPanel.editor.Tag).Text = result;
...
*/
    ...
    */
    ...
    public static void BrowseEx(String path)
    {
      /*
    ...
    {
      NodeInfo ni = menuTree.treeView.SelectedNode.Tag as NodeInfo;
      if (File.Exists(ni.Path)) return ni.Path;
      else if (File.Exists(AntPanel.editor.currentPath))
      {
        return AntPanel.editor.currentPath;
      }
      return String.Empty;
    }
