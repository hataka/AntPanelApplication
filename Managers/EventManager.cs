	...
	public class EventManager
	{
		...
		public static void AddEventHandler(IEventHandler handler, EventType mask, HandlingPriority priority)
		{
			...
			catch (Exception ex)
			{
			  ...
			  "AddEventHandler(IEventHandler handler, EventType mask, HandlingPriority priority)");
		...
		public static void RemoveEventHandler(IEventHandler handler, EventType mask, HandlingPriority priority)
		{
			...
			{
		MessageBox.Show(Lib.OutputError(ex.Message.ToString()),
"RemoveEventHandler(IEventHandler handler, EventType mask, HandlingPriority priority)");
		//ErrorManager.ShowError(ex);
	  }
	}
		...
		public static void RemoveEventHandler(IEventHandler handler)
		{
			...
			{
		MessageBox.Show(Lib.OutputError(ex.Message.ToString()),
		  "RemoveEventHandler(IEventHandler handler)");
			  //ErrorManager.ShowError(ex);
			}
		}
	  ...
	  try
	  {
		...
		//MessageBox.Show(de.Action);
		Globals.MainForm.HandleEvent(sender, e, HandlingPriority.Normal);
	  }
	  catch { }
	  ...
	  try 
			{
				...
				{
					List<EventObject> subObjects = objectList[i];
		  //MessageBox.Show(subObjects.Count.ToString(), "Dispatch");
		  for (Int32 j = 0; j < subObjects.Count; j++)
					{
						EventObject obj = subObjects[j];
						if ((obj.Mask & e.Type) > 0)
						{
							obj.Handler.HandleEvent(sender, e, (HandlingPriority)obj.Priority);
							if (e.Handled) return;
						}
					}
				}
	  ...
	  {
		MessageBox.Show(Lib.OutputError(ex.Message.ToString()),"DispatchEvent(Object sender, NotifyEvent e)");
